...
async function fetchorderFromShopify(session, cursor = null) {
    ...
    if (cursor != "null") {
        query = {
            limit: 10,
            fields: "order_number,created_at,line_items,total-price,customer,currency",
            page_info: cursor,
        };
    } else {
        query = {
...
export function formateOrderData(data) {
    ...
    const transformedData = data.map((order) => {
        ...
        const footprintReduction = order.line_items.find(
            (item) =>
                item.properties &&
                item.properties.some((prop) => prop.name === "lable" && prop.value === "co2-with-Emissa")
        );

        const co2FootPrint = order.line_items.find(
            (item) => item.properties && item.properties.some((prop) => prop.name === "footprint")
        );

        return [
            `# ${order.order_number}`,
            order.created_at.split("T")[0],
            co2FootPrint ? co2FootPrint.properties.find((prop) => prop.name === "footprint").value : "",

        ...
        for (let i = 0; i < products.data.length; i++) {
            ...
            if (x.vendor !== "Emissa") {
                for (let j = 0; j < x.variants.length; j++) {
                    ...
                    const compensationData = metafield.data?.find((x) => x.key === "co2compensation");
...
export const importProducts = async (req, res) => {
    ...
    for (const row of data) {
        console.log("row Co2 Compensation:", row["Co2 Compensation"]);
        console.log("row Co2 Footprints:", row["Co2 Footprints"]);
        console.log("row=>", row);
        // addMetafieldToVariant(session, row, shop.data[0]);
        await limiter.schedule(() => addMetafieldToVariant(session, row, shop.data[0]));
    }
    ...
    try {
        ...
        const data = await client.get({
            ...
            query: {
                limit: 50,
                tag: "co2compensation",
                fields: "id,  order_number,created_at,line_items,total-price,customer,currency",
                since_id: req.query.since_id,
            },
        });
...
async function addMetafieldToVariant(session, data, shop) {
    ...
    } else if (data["Co2 Footprints"] && data["Co2 Footprints"] != "" && data["Co2 Footprints"] != 0) {
        co2 = await metafield(session, data["Co2 Footprints"], co2footprints?.id, co2footprints?.owner_id, "create", {
    ...
    if (isNaN(data["Co2 Compensation"]) === false) {
        if (co2compensation) {
            ...
            co2comp = await metafield(session, price, co2compensation?.id, co2compensation?.owner_id, "update", {});
        } else if (data["Co2 Compensation"] && data["Co2 Compensation"] !== "" && data["Co2 Compensation"] != 0) {
            const price = JSON.stringify({
                amount: data["Co2 Compensation"],
                currency_code: shop.currency,
            });
            co2comp = await metafield(session, price, co2compensation?.id, co2compensation?.owner_id, "create", {
